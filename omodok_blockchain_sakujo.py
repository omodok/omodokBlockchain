...
def wsServerFn(ws):
    ...
    def wsSkujoFn(s):            
        global nodes, ws_list, ip_dict, wsc_list, ipc_dict
        ws_list.remove(s)
        nodes.remove(ip_dict[s])
        print("pop_ip_list:",ip_dict[s])
        ip_dict = ip_dict.pop(s)
...
async def wsClientFn(wsURL):
    global newNode, wsc_list, ipc_dict, invFlag, TxFlag, bcSoushin
    
    def wscSakujoFn(c):
        global wsc_list, ipc_dict, nodes
        print("ここまでOK-C")
        wsc_list.remove(c)
        ipc_dict.pop(c)
        #nodes.remove(wsURL)
        ...
        while True:
            ...
            try:
                ...
                if recvJson.get("addr") != None:
                    if newNodeFn(recvJson["addr"]):
                        ip_dict[ws] = recvJson["addr"]                    
            ...
            if TxFlag and wsURL not in bcSoushin:
                ...
                print("CA>>>Ping",wsURL,len(wsc_list))
                try:
                    wsc.send("Ping")
                except:
                    wscSakujoFn(wsc)
