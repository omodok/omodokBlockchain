...
def newNodeFn(newNode,ws):
    global nodes,ip_dict
    if ws != "":
        ip_dict[ws]=newNode
    for node in nodes:
        if node == newNode:
...
async def wsClientFn(wsURL):
    ...
    def wscSakujoFn(s):            
        global nodes, wsc_list, ipc_dict
        wsc_list.remove(s)
        nodes.remove(ipc_dict[s])
        print("pop_ip_list:",ipc_dict[s])
        ipc_dict.pop(s)
